...
protected:
    ...
    bool shoot(player* victim, player* hunter, string cmd, crocodile* croc)
    {
        if ((*hunter).bullets == 0)
        {
    ...
    bool explode(player* player, string cmd)
    {
    ...
    bool step(player* pl, int turns, int* time_in_swamp, int id, crocodile* croc, player* pl1, player* pl2)
    {
        ...
        if (command == "Shoot")
        {
            ...
            {
                trash = shoot(&(*pl2), pl, command, croc);
            ...
            {
                trash = shoot(&(*pl1), pl, command, croc);
                if ((lines[(*pl).coord.i][(*pl).coord.j] == 's' or
                    lines[(*pl).coord.i][(*pl).coord.j] == 'u')
                    and (*croc).alive)
                {
...
public:
    Maze()
    {
        ...
        cin >> action;
        if (action == "Yes" or action == "y" or action == "yes") respawn_new_place = 0; // в будущем часть функции init_settings
        else respawn_new_place = 1;
